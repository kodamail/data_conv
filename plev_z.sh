...
#    PDEF=$( get_pdef ${PDEF_LEVELS} ) || exit 1
	...
	sed ${OUTPUT_CTL}.tmp1 \
	    ...
	    -e "s/^${VAR_PRES}.*$/${VAR} ${PDEF} 99 geopotential height/" \
...
#	let TMAX=d+TDEF_FILE-1
	...
	if [ "${START_HMS}" = "000000" ] ; then
        ...
        grads_get_data.sh ${VERBOSE_OPT} ${INPUT_CTL} ${VAR_PRES} ${TEMP_DIR}/${VAR_PRES}_${YMD}.grd.in \
            -ymd "${YMD_TMP}" || exit 1   
	#
	# create z on z
	#
...
'xopen ${INPUT_CTL}'
'set gxout fwrite'
'set fwrite -be ${TEMP_DIR}/${VAR}_${YMD}.grd.in'
'set undef dfile'
'set x 1 ${XDEF}'
'set y 1 ${YDEF}'
